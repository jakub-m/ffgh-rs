...
pub fn print_pull_requests<W: Write>(
    ...
    prs.sort_by(|a, b| {
        let a_priority = display_priority
            .get(&a.meta.label)
            .copied()
            .unwrap_or(usize::MAX);
        let b_priority = display_priority
            .get(&b.meta.label)
            .copied()
            .unwrap_or(usize::MAX);
    ...
    let filtered_prs = match user_state.settings.view_mode.as_str() {
        VIEW_MODE_MUTE_TOP => {
            let mut not_muted: Vec<_> = prs
                .iter()
                .filter(|pr| !is_mute(user_state, pr))
                .cloned()
                .collect();
            let muted: Vec<_> = prs
                .iter()
    ...
    for pr in &filtered_prs {
        ...
        let line = format!(
            "{}\t{}",
            pr.url,
            join_strings_cap_width(terminal_width, &line_left, &line_right)
        );
...
pub fn print_show_pull_request<W: Write>(
    ...
    if let Some(pr) = pr {
        ...
        let details = vec![
            ...
            String::new(),
            flag_string,
            format!("{} ({})", pr.author.login, pr.meta.label)
                .yellow()
                .to_string(),
            format!(
                "Created {}, updated {} ago",
